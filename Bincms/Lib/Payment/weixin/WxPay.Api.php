...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url(WEIXIN_NOTIFY_URL);//异步通知url
		}
		
		$inputObj->SetAppid(WEIXIN_APPID);//公众账号ID
		$inputObj->SetMch_id(WEIXIN_MCHID);//商户号
	...
	{
		//如果不需要上报数据
		if(WEIXIN_REPORT_LEVENL == 0){
			return;
		} 
		//如果仅失败上报
		if(WEIXIN_REPORT_LEVENL == 1 &&
	...
	{		
		...
		//如果有配置代理这里就设置代理
		if(WEIXIN_CURL_PROXY_HOST != "0.0.0.0" 
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, WEIXIN_SSLCERT_PATH);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, WEIXIN_SSLKEY_PATH);
		}
		//post提交方式
