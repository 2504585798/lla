...
class TiebaAction extends CommonAction {
    ...
    public function index(){
		...
		$ding = $Post->where(array('orderby'=>'0'))->order('post_id desc')->limit(0,5)->select();
	...
	$map = array('post_id' => $post_id);// 不用审核直接显示
       // $map = array('audit' => 1,'post_id' => $post_id);// 审核后才显示
    ...
    public function reply($post_id) {
        ...
        if ($this->isPost()) {
			//if ($data['safecode']!=session('safecode')) {			//回帖防灌水安全认证
				//$this->niuMsg('不能重复发帖哦，正在为您跳转……',U('tieba/post',U('tieba/detail', array('post_id' => $post_id))));
			//}
            ...
            if ($obj->add($data)) {
				...
				$this->niuMsg('回帖成功啦！',U('tieba/detail', array('post_id' => $post_id)));
            }
            $this->niuMsg('操作失败！');
        } else {
            $this->assign('detail', $detail);
    ...
    public function post() {
        ...
        if ($this->isPost()) {
			...
			$data['details'] = $tupian[contents] . $photo;
			if ($data['safecode']!=session('safecode')) {		//发帖防灌水安全认证
				$this->niuMsg('不能重复发帖哦，正在为您跳转……',U('tieba/post'));
			}
			if (empty($data['cate_id'])) {
				$this->niuMsg('分类不能为空');
			}
			//$this->niuMsg($tupian['title'] . $data['pic']);die;
			$last = $obj->add($data);
            if ($last) {
				session('safecode',null);
				$this->niuMsg('发帖成功啦！',U('tieba/detail', array('post_id' => $last)));
            }
            $this->niuMsg('操作失败！');
        } else {
			$this->assign('cate', $cate);
