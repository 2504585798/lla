...
class MessageAction extends CommonAction {
	...
	public function post($msg_id){
		...
		if ($this->isPost()) {
			...
			$data['send_id'] = $detail['user_id'];
			$data['content'] = $this->_param('content');
			$data['create_time'] = time();
			
			if($data['content']==''){
				$this->niuMsg('信息内容不能为空！');
				die;
			}
			$Message->add($data);
			$this->niuMsg('信息发送成功！', U('message/detail',array('msg_id'=>$detail['msg_id'])));
		}else{
			$this->assign('detail', $detail); // 赋值数据集
			$this->display(); // 输出模板
		}
	...
	public function send($user_id){
		if (empty($this->uid)) {
			$this->niuMsg('您还没有登录！', U('passport/login'));
		}
		...
		if ($this->isPost()) {
			...
			if($last){
				$this->niuMsg('信息发送成功！', U('message/index'));
			}else{
				$this->niuMsg('信息发送失败！');
			}
		}else{
