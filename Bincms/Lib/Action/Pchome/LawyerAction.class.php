...
class LawyerAction extends CommonAction {
    ...
    public function pay2() {
        ...
        if ($code == 'wait') { //如果是货到付款
           ...
           $_data_order = array(//整体变更
            ...
            $this->ajaxReturn(array('status' => 'success', 'msg' => '恭喜您下单成功！', 'url' => U('member/ele/index')));
        } else {
    ...
    public function evaluate() {
        ...
        $count1 = $shopdianping->where(array('closed' => 0, 'shop_id' => $shop_id, 'show_date' => array('ELT', TODAY), 'evaluate' => 1))->count();
        $count2 = $shopdianping->where(array('closed' => 0, 'shop_id' => $shop_id, 'show_date' => array('ELT', TODAY), 'evaluate' => 2))->count();
        $count3 = $shopdianping->where(array('closed' => 0, 'shop_id' => $shop_id, 'show_date' => array('ELT', TODAY), 'evaluate' => 3))->count();
        ...
        $this->assign('list', $list); // 赋值数据集--点评
        $this->assign('page', $show); // 赋值分页输出 -- 点评
        $this->assign('detail', $detail);
        $this->assign('shop', $shop);
        $this->assign('linkArr', $linkArr);
        $this->assign('ex', D('Shopdetails')->find($shop_id));
