...
class CommonAction extends Action {
    ...
    protected function _initialize() {
       ...
       /*  $this->citys = D('City')->fetchAll();
        ...
        if (!empty($this->uid)) {
            $memberhost=$this->member = D('Users')->find($this->uid);
        }
		...
		$this->assign('doctor',$this->doctor=D('Doctor')->where(array("user_id"=>$this->uid))->find());
		$this->assign('hospitalcenter',$this->hospitalcenter=D('Hospital')->where(array("user_id"=>$this->uid))->find());
		$user_rank=D('Userrank')->fetchall();//用户等级
       ...
       // $this->assign('itemcates',D('Itemcate')->fetchAll());
	...
	$chinaarea=D('Chinarea')->where(array('parent_id'=>1))->select();//地区顶级分类
	$this->assign('chinaarea', $chinaarea);
	 //dump($chinaarea);
