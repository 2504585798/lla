...
class HospitalcateAction extends CommonAction {
    ...
    public function index() {
        $Hospitalcate = D('Hospitalcate');
        $list = $Hospitalcate->fetchAll();
    ...
    public function create($parent_id=0) {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->baoSuccess('添加成功', U('hospitalcate/index'));
            }
            $this->baoError('操作失败！');
        } else {
    ...
    public function delete($cate_id = 0) {
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            $obj->delete($cate_id);
            $obj->cleanCache();
            $this->baoSuccess('删除成功！', U('hospitalcate/index'));
        } else {
    ...
    public function update() {
        ...
        $obj->cleanCache();
        $this->baoSuccess('更新成功', U('hospitalcate/index'));
    }
