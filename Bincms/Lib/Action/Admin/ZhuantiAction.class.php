...
class ZhuantiAction extends CommonAction 
{
    ...
    {
        $count = D('Zhuanmap')->count(); // 查询满足要求的总记录数 
        import('ORG.Util.Page'); // 导入分页类
        $Page  = new Page($count, 15); // 实例化分页类 传入总记录数和每页显示的记录数
        $show  = $Page->show(); // 分页显示输出
    ...
    public function editGoods($zhuan_id=null)
    {
        ...
        }else if(!$zhuan=M('Zhuan')->find($zhuan_id)){
            $this->baoError('该商品未添加在或已移除！');
        }else{
            $floors = D('Zhuanfloor')->order('sort asc')->select();
            $this->assign('floors',$floors);
    ...
    {
        $count = D('Zhuanfloor')->count(); // 查询满足要求的总记录数 
        import('ORG.Util.Page'); // 导入分页类
        $Page = new Page($count, 15); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
    ...
    public function editfloor($floor_id=null)
    {
        if($data=I('post.data')){
            ...
            $this->baoError('参数不正确');
        }else if(!$floor=D('Zhuanfloor')->find($floor_id)){
    ...
    public function delFloor($floor_id=null)
    {
        ...
        if(!$floor_id){
            $this->baoError('参数不正确！');
        }else if(M('Zhuan')->where("floor_id={$floor_id}")->find()){
