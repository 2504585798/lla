...
class HospitalnewsAction extends CommonAction {

    private $create_fields = array('hospital_id', 'title', 'photo', 'details', 'views', 'orderby');
    private $edit_fields = array('hospital_id', 'title', 'photo', 'details', 'views', 'orderby');

    public function index() {
        $Hospitalnews = D('Hospitalnews');
       ...
       if($cate_id = (int)$this->_param('cate_id')){
        ...
        $count = $Hospitalnews->where($map)->count(); // 查询满足要求的总记录数 
        $Page = new Page($count, 25); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
        $list = $Hospitalnews->where($map)->order(array(  'news_id'=>'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $hospital_ids = array();
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Hospitalnews');
            if ($obj->add($data)) {
                $this->baoSuccess('添加成功', U('hospitalnews/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->display();
    ...
    public function edit($news_id = 0) {
        if ($news_id = (int) $news_id) {
            ...
            if (!$detail = $obj->find($news_id)) {
                $this->baoError('请选择要编辑的医院资讯');
            }
            if ($this->isPost()) {
                $data = $this->editCheck();
                $data['news_id'] = $news_id;
                if (false !== $obj->save($data)) {
                    $this->baoSuccess('操作成功', U('hospitalnews/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    public function delete($news_id = 0) {
        if (is_numeric($news_id) && ($news_id = (int) $news_id)) {
            $obj = D('Hospitalnews');
            $obj->delete($news_id);
            $this->baoSuccess('删除成功！', U('hospitalnews/index'));
        } else {
    ...
    public function audit($news_id = 0) {
        if (is_numeric($news_id) && ($news_id = (int) $news_id)) {
            ...
            $this->baoSuccess('审核成功！', U('hospitalnews/index'));
        } else {
            $news_id = $this->_post('news_id', false);
            if (is_array($news_id)) {
                $obj = D('Hospitalnews');
