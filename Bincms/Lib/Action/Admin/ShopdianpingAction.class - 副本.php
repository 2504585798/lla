...
class ShopdianpingAction extends CommonAction {
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($dianping_id = $obj->add($data)) {
                ...
                if(!empty($local)) D('Shopdianpingpics')->upload($dianping_id,$data['shop_id'],$local);
                $this->baoSuccess('添加成功', U('shopdianping/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->display();
        ...
        if ($dianping_id = (int) $dianping_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    if(!empty($local)) D('Shopdianpingpics')->upload($dianping_id,$data['shop_id'],$local);
                    $this->baoSuccess('操作成功', U('shopdianping/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    public function delete($dianping_id = 0) {
        if (is_numeric($dianping_id) && ($dianping_id = (int) $dianping_id)) {
            ...
            $this->baoSuccess('删除成功！', U('shopdianping/index'));
        } else {
