...
class ChargeAction extends CommonAction {
    ...
    public function pay(){
        ...
        if (IS_POST) {
            ...
            if ($member['banlance'] < $total) {
                $this->error('很抱歉您的账户余额不足', U('/mcenter/money'));
            }
            if ($member['interest_banlance'] >= $total) {
                $logMoney = 0;
                $logInterest = $total;
                $member['used_interest'] += $total;
            }
            else {
                $logMoney = $total - $member['interest_banlance'];
                $logInterest = $member['interest_banlance'];
