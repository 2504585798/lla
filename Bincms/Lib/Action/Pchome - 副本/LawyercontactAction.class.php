...
class  LawyercontactAction extends CommonAction{
    ...
    public function detail(){
		...
		//回复列表
		import('ORG.Util.Pageabc'); // 导入分页类
		$count =  D('Shopcomment')->where(array('post_id'=>$news_id,'parent_id'=>0))->count(); //获取评论总数
		$Page = new Page($count, 15); // 实例化分页类 传入总记录数和每页显示的记录数
		$show = $Page->show(); // 分页显示输出
		$this->assign('count',$count);
		$list=array();
		$list=$this->getCommlist($news_id,0,$Page->firstRow,$Page->listRows);//获取评论列表
    ...
    public function zans() {
        ...
        $this->niuSuccess('恭喜您，点赞成功！', U('seller/detail', array('news_id' => $detail['post_id'])));
    }
	...
	public function post(){
		...
		if (D('Shopcomment')->add($data)) {
			$this->niuSuccess('回复成功！', U('seller/detail', array('news_id' => $data['post_id'])));
		}
	}
