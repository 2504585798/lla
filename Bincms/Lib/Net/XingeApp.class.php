...
class PushService{
	 ...
	 */
	public function PushAllAndroid($title, $content,$url)
	{
		$ret = XingeApp::PushAllAndroid($this->accessId,$this->secretKey,$title,$content);
	...
	public function PushAllIos($title, $content)
	{
		$mess = new MessageIOS();
		$mess->setAlert($content);
		$environment = XingeApp::IOSENV_DEV;
		$ret = $this->XingeApp->PushAllDevices(0, $mess, $environment);
		return $ret;
	}

	public function PushSingleAndroid($tile,$content,$uid,$url){
		$ret = XingeApp::PushAccountAndroid($this->accessId, $this->secretKey, $title, $content, $uid,$url);
		if(!empty($ret['err_msg'])){
			return false;
		}else{
			return true;
	...
	public function PushSingleIos($tile,$content,$uid){
		$environment = XingeApp::IOSENV_DEV; //TODO
		$ret = XingeApp::PushAccountIos($this->accessId, $this->secretKey, $title, $content, $uid,$environment);
		if(!empty($ret['err_msg'])){
			return false;
		}else{
			return true;
...
class XingeApp {
	 ...
	 */
	public static function PushAccountAndroid($accessId, $secretKey, $title, $content, $account,$url)
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new Message();
		$mess->setTitle($title);
	 ...
	 * 使用默认设置推送消息给所有设备android版
	 */
	public static function PushAllAndroid($accessId, $secretKey, $title, $content,$url)
	{
		$push = new XingeApp($accessId, $secretKey);
		$mess = new Message();
		$mess->setTitle($title);
