<?php
class Jpush {
        private $app_key = 'dd22d94472ae965a398a3573';        //待发送的应用程序(appKey)，只能填一个。
        private $master_secret = '09cd2d8b0093709ac43976c8';    //主密码
        private $url = "https://api.jpush.cn/v3/push";      //推送的地址
    
		 /**
	     * 构造方法，用于实例化极光SDK
	     * @param string $token 极光开放平台设置的TOKEN
	     */
	    public function __construct() {
	        $where = array('k'=>'jpush');
	        $ret = D('setting')->where($where)->find();
	        if($ret){
	        	$ret = unserialize($ret['v']);
				$this->app_key = $ret['appkey'];
		        $this->master_secret = $ret['secret'];
	        }else{
	        	$this->app_key ='';
	        	$this->master_secret ='';
	        }
        ...
        **/
        //调用推送方法
        public function send_pub($receive,$content,$m_type,$m_txt,$m_time=600){
